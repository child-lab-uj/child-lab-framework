...
def to_3D(fx: float, fy: float, depth: np.ndarray, cx: float, cy: float, u: np.ndarray, v: np.ndarray) -> np.ndarray:
    """
...
def make_point_cloud(rgbd: np.ndarray, fx: float, fy: float, cx: float, cy: float) -> np.ndarray:
    """
