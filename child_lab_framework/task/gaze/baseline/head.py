...
class Result:
    ...
    def visualize(
        ...
        draw_mask = configuration.gaze_head_baseline_draw_mask
        draw_centers = configuration.gaze_head_baseline_draw_centers
        mask_color = configuration.gaze_head_baseline_mask_color
        mask_opacity = configuration.gaze_head_baseline_mask_opacity
        gaze_color = configuration.gaze_baseline_line_color
        gaze_thickness = configuration.gaze_line_thickness

        solid_color = np.zeros_like(frame, dtype=np.uint8)
        solid_color[:] = np.array(mask_color[:3], dtype=np.uint8)
...
def estimate(
    ...
    for i in range(n_actors):
        ...
        depths = depth[baseline_head_masks[i, ...]]

        if len(depths) == 0:
            actor_visibility_mask[i] = False
            continue

        quantile_thresholds[i] = np.quantile(depths, q=quantile)
