...
# | File: `src/depth_pro/network/encoder.p                              |
...
def _project_upsample_block(
    ...
    intermediate: int
        Number of features between the first and second block in sequence.
    """
    ...
    upsampling = [
        ...
        for _ in range(1, upsampling_layers)
    ]
...
class Encoder(nn.Module):
    """
    DepthPro Encoder, aimed at creating multi-resolution encodings from Vision Transformers.
    """
    ...
    def __init__(
        ...
        ---
        decoder_features: int
            Number of feature output in the decoder.

        stage_dimensions: list[int]
            Dimensions of the encoder at different layers.

        hook_block_ids: list[int]
            Hooks to obtain intermediate features for the patch encoder model.

        patch_encoder: torch.nn.Module
            Backbone used for patches.

        image_encoder: torch.nn.Module
            Backbone used for global image encoder.
        """
        ...
        # These parameters are ints in the official checkpoint.
        patch_encoder_embed_dim: int = patch_encoder.embed_dim  # type: ignore
        image_encoder_embed_dim: int = image_encoder.embed_dim  # type: ignore

        # These parameters exist in the official checkpoint.
        self.output_size = int(
            patch_encoder.patch_embed.img_size[0]  # type: ignore
            // patch_encoder.patch_embed.patch_size[0]  # type: ignore
        )
        ...
        patch_encoder.blocks[hook_block_ids[0]].register_forward_hook(self.hook0)  # type: ignore
        patch_encoder.blocks[hook_block_ids[1]].register_forward_hook(self.hook1)  # type: ignore
    ...
    def hook0(
        self,
        _model: timm.models.vision_transformer.Block,
    ...
    def image_size(self) -> int:
        ...
        return int(self.patch_encoder.patch_embed.img_size[0]) * 4  # type: ignore
